...
import { chapter6sr } from "@/app/data/sr/chapter-6";
import { chapter6en } from "@/app/data/en/chapter-6";
import { toRoman } from "@/app/utils/toRoman";
import FooterChapters from "@/app/components/FooterChapters";
import { useLanguage } from "@/app/context/LanguageContext";
import HomeButtonBlack from "@/app/components/HomeButtonBlack";
import CreditsButton from "@/app/components/Credits";

function page() {
  const { language } = useLanguage();
  const chapter = language === "sr" ? chapter6sr : chapter6en;
  ...
  before:bg-[url('/images/background/15.webp')]
          ...
          {chapter.pages.map((page, index) => (
            ...
            >
              ...
              <div className="absolute w-full h-screen bg-no-repeat bg-cover">
                <Image
                  src={`/images/line/chapter6-0${index + 1}.png`}
                  alt=""
                  fill
                  priority
                ...
                {index === 0 && (
                  <>
                    ...
                    <h1 className="absolute flex flex-col text-(--imscrvena) text-5xl mt-85 ml-20">
                      {language === "sr" ? (
                        <>
                          <span className="ml-10">Жена</span>
                          <span className="ml-18"> и путовање</span>
                        </>
                      ) : (
                        <>
              ...
              <PhotoAnimation index={index}>
                {page.image && (
                  ...
                  >
                    <Image
                      src={`/images/chapter-6/0${page.image.name}.webp`}
                      alt=""
              ...
              <ButtonAnimation index={index}>
                ...
                {/*   {index === chapter.pages.length - 1 && <HomeButtonBlack />} */}
              </ButtonAnimation>
              <TextAnimation index={index}>
                {index === chapter.pages.length - 1 && <CreditsButton />}
              </TextAnimation>
            </CarouselItem>
          ))}
        </CarouselContent>
